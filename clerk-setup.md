...
## Nota para EventRank (contexto del proyecto)

- App de Clerk ya creada: `app_3FkM9JG1BXuiMoI1yeoqJOgsogL`. Pasar `--app app_3FkM9JG1BXuiMoI1yeoqJOgsogL` en `clerk init`.
- Los usuarios (super_admin + organizadores del CEAL) se crean en el dashboard de Clerk, con `publicMetadata`: `{ "rol": "...", "tenant_id": "<uuid>" }`.
- Para que RLS de Supabase funcione con Clerk: crear un JWT template "supabase" en el dashboard de Clerk, y usar la función `requesting_user_id()` en las políticas (lee el `sub` del JWT). Ver `agentdocs/specs/auth.spec.md`.
